...
public class POSPanel extends JPanel {
	...
	JButton[] SBtn = new JButton[5];
	String[] Str = {"주문확인","품목취소","전체취소","주문완료","결제"};
	...
	public POSPanel(JFrame f, int tableNum) {
		this.f = f;
		...
		for(int i=0;i<MBtn.length;i++) {
			...
			MBtn[i].addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					Menu menu = new Menu(FoodMenu[index],count,Menuprice[index]);
					POS_Main.arr.add(menu);
				}
			});
		}
		//음료주문
		for(int i=0;i<DBtn.length;i++) {
			...
			DBtn[i].addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent d) {
					...
					Menu menu = new Menu(DrinkMenu[index],count,Drinkprice[index]);
					POS_Main.arr.add(menu);
				}
			});
		}
		...
		//주문완료
			SBtn[3].addActionListener(new ActionListener() {
				...
				POS_Main.tableArr[tableNum]=POS_Main.arr;
				 frame = new POSTableFrame();
