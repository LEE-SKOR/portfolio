...
-- 데이터 사이즈 수정
ALTER TABLE E_EMP
MODIFY (제작자 VARCHAR2(200)); -- 원래 (100) => (200) 사이즈로 변경
DESC e_emp;

ALTER TABLE e_emp
MODIFY (제작자 VARCHAR2(50)); -- 원래 (200) => (50) 사이즈로 변경

ALTER TABLE e_emp
MODIFY (제작자 VARCHAR2(1)); -- 원래 (50) => (1) 사이즈로 변경 에러발생
ALTER TABLE e_emp
MODIFY (제작자 NUMBER); -- 이미 문자형 데이터가 있기때문에 숫자형으로 바꿀 수 없다.

SELECT * FROM e_emp;
DESC e_emp;

-- NULL값인 경우에 입력된 값이 없기때문에 데이터 형식을 바꿀 수 있다.
ALTER TABLE e_emp
MODIFY (GENDER NUMBER);
-- 제작자의 데이터 형식을 바꾸려 하면
UPDATE E_EMP
SET 제작자 = NULL;
-- NULL 값일때 데이터 크기는 수정이 가능하지만
ALTER TABLE e_emp
MODIFY (제작자 VARCHAR2(1));
-- 제작자 형식을 숫자로 변경은 불가능하다
ALTER TABLE e_emp
MODIFY (제작자 NUMBER);
